...
public class SandFishEntity extends Monster {
        ...
        public boolean canUse() {
            if (this.mob.getTarget() != null) {
                ...
                if (/*net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.mob.level(), this.mob) &&*/ randomsource.nextInt(reducedTickDelay(10)) == 0) {
                    ...
                    if (/*InfestedBlock.isCompatibleHostBlock(blockstate)*/!blockstate.isAir()) {
                        this.doMerge = true;
                        return true;
                    }
                }
    ...
    static class SandfishWakeUpFriendsGoal extends Goal {
        ...
        public void tick() {
            --this.lookForFriends;
            if (this.lookForFriends <= 0) {
                Level level = this.sandFish.level();
                RandomSource randomsource = this.sandFish.getRandom();
                BlockPos blockpos = this.sandFish.blockPosition();
                    ...
                    for(int j = 0; j <= 10 && j >= -10; j = (j <= 0 ? 1 : 0) - j) {
                        for(int k = 0; k <= 10 && k >= -10; k = (k <= 0 ? 1 : 0) - k) {
                            ...
                            if (block instanceof InfestedBlock) {
                                if (true/*net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(level, this.sandfish)*/) {
                                    level.destroyBlock(blockpos1, true, this.sandFish);
                                } else {
