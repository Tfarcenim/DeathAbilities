...
public class ElementalDragonEntity extends Mob implements Enemy {
    ...
    private final ElementalDragonPart[] subEntities;
    public final ElementalDragonPart head;
    private final ElementalDragonPart neck;
    private final ElementalDragonPart body;
    private final ElementalDragonPart tail1;
    private final ElementalDragonPart tail2;
    private final ElementalDragonPart tail3;
    private final ElementalDragonPart wing1;
    private final ElementalDragonPart wing2;
    ...
    @Nullable
    private EndElementalDragonFight dragonFight;
    private BlockPos fightOrigin = BlockPos.ZERO;
    private final ElementalDragonPhaseManager phaseManager;
    ...
    public ElementalDragonEntity(EntityType<? extends ElementalDragonEntity> pEntityType, Level pLevel) {
        ...
        this.tail1 = new ElementalDragonPart(this, "tail", 2.0F, 2.0F);
        this.tail2 = new ElementalDragonPart(this, "tail", 2.0F, 2.0F);
        this.tail3 = new ElementalDragonPart(this, "tail", 2.0F, 2.0F);
        this.wing1 = new ElementalDragonPart(this, "wing", 4.0F, 2.0F);
        this.wing2 = new ElementalDragonPart(this, "wing", 4.0F, 2.0F);
        this.subEntities = new ElementalDragonPart[]{this.head, this.neck, this.body, this.tail1, this.tail2, this.tail3, this.wing1, this.wing2};
        this.setHealth(this.getMaxHealth());
        this.noPhysics = true;
        this.noCulling = true;
        this.phaseManager = new ElementalDragonPhaseManager(this);
    ...
    public void setDragonFight(EndElementalDragonFight pDragonFight) {
    ...
    public void aiStep() {
        ...
        if (this.dragonFight == null) {
            ...
            if ($$6 instanceof ServerLevel serverlevel) {
                EndElementalDragonFight enddragonfight = ServerLevelDuck.of(serverlevel).getFight();
        ...
        if (this.isDeadOrDying()) {
            ...
            if (this.isNoAi()) {
                ...
                if (this.level().isClientSide) {
                    ...
                    ElementalDragonPhaseInstance dragonphaseinstance = this.phaseManager.getCurrentPhase();
                ...
                this.tickPart(this.wing1, f15 * 4.5F, 2.0D, f1 * 4.5F);
                this.tickPart(this.wing2, f15 * -4.5F, 2.0D, f1 * -4.5F);
                ...
                for (int k = 0; k < 3; ++k) {
                    ElementalDragonPart enderdragonpart = null;
                    if (k == 0) {
                    ...
                    this.tickPart(enderdragonpart, -(f1 * 1.5F + f18 * f22) * f13, adouble1[1] - adouble[1] - (double) ((f22 + 1.5F) * f) + 1.5D, (f15 * 1.5F + f20 * f22) * f13);
                }
    ...
    private void tickPart(ElementalDragonPart pPart, double pOffsetX, double pOffsetY, double pOffsetZ) {
            ...
            for (int l1 = j; l1 <= i1; ++l1) {
                for (int i2 = k; i2 <= j1; ++i2) {
                    ...
                    if (!blockstate.isAir() && !blockstate.is(BlockTags.DRAGON_TRANSPARENT)) {
                        if (Services.PLATFORM.canEntityDestroyHook(this.level(), blockpos, this) && !blockstate.is(BlockTags.DRAGON_IMMUNE)) {
    ...
    public boolean hurt(ElementalDragonPart pPart, DamageSource pSource, float pDamage) {
        if (this.phaseManager.getCurrentPhase().getPhase() == ElementalDragonPhase.DYING) {
            return false;
        } else {
            ...
            if (pDamage < 0.01F) {
                ...
                if (pSource.getEntity() instanceof Player || pSource.is(DamageTypeTags.ALWAYS_HURTS_ENDER_DRAGONS)) {
                    ...
                    if (this.isDeadOrDying() && !this.phaseManager.getCurrentPhase().isSitting()) {
                        this.setHealth(1.0F);
                        this.phaseManager.setPhase(ElementalDragonPhase.DYING);
                    }
                    ...
                    if (this.phaseManager.getCurrentPhase().isSitting()) {
                        ...
                        if (this.sittingDamageReceived > 0.25F * this.getMaxHealth()) {
                            ...
                            this.phaseManager.setPhase(ElementalDragonPhase.TAKEOFF);
                        }
                    }
                }
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        return !this.level().isClientSide && this.hurt(this.body, pSource, pAmount);
    }
    ...
    protected void tickDeath() {
        ...
        if (this.level() instanceof ServerLevel) {
            if (this.dragonDeathTime > 150 && this.dragonDeathTime % 5 == 0 && flag) {
                int award = Services.PLATFORM.getExperienceDropHook(this, this.unlimitedLastHurtByPlayer, Mth.floor(i * 0.08F));
        ...
        if (this.dragonDeathTime == 200 && this.level() instanceof ServerLevel) {
            if (flag) {
                int award = Services.PLATFORM.getExperienceDropHook(this, this.unlimitedLastHurtByPlayer, Mth.floor(i * 0.2F));
                ExperienceOrb.award((ServerLevel) this.level(), this.position(), award);
            }
    ...
    public ElementalDragonPart[] getSubEntities() {
    ...
    public float getHeadPartYOffset(int pPartIndex, double[] pSpineEndOffsets, double[] pHeadPartOffsets) {
        ElementalDragonPhaseInstance dragonphaseinstance = this.phaseManager.getCurrentPhase();
        ElementalDragonPhase<? extends ElementalDragonPhaseInstance> enderdragonphase = dragonphaseinstance.getPhase();
        double d0;
        if (enderdragonphase != ElementalDragonPhase.LANDING && enderdragonphase != ElementalDragonPhase.TAKEOFF) {
    ...
    public void onCrystalDestroyed(EndCrystal pCrystal, BlockPos pPos, DamageSource pDamageSource) {
        ...
        if (pDamageSource.getEntity() instanceof Player) {
            ...
            player = this.level().getNearestPlayer(CRYSTAL_DESTROY_TARGETING, pPos.getX(), pPos.getY(), pPos.getZ());
        }
    ...
    public void onSyncedDataUpdated(EntityDataAccessor<?> pKey) {
        if (DATA_PHASE.equals(pKey) && this.level().isClientSide) {
            this.phaseManager.setPhase(ElementalDragonPhase.getById(this.getEntityData().get(DATA_PHASE)));
        }

        super.onSyncedDataUpdated(pKey);
    }

    public ElementalDragonPhaseManager getPhaseManager() {
        return this.phaseManager;
    }

    @Nullable
    public EndElementalDragonFight getDragonFight() {
